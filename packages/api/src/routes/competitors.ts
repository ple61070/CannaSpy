...
export async function competitorsRoutes(fastify: FastifyInstance) {
  ...
  fastify.post('/', async (req: FastifyRequest, reply: FastifyReply) => {
    ...
    // Select-then-insert: avoids ON CONFLICT partial-index inference issues
    if (google_place_id) {
    ...
    try {
      ...
      // 23505 = unique_violation — concurrent insert race; fetch the row that won
      if (err.code === '23505' && google_place_id) {
        const existing = await query<{ id: string }>(
          'SELECT id FROM competitors WHERE google_place_id = $1 LIMIT 1',
          [google_place_id]
        )
