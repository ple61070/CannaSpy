  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', background: 'var(--bg)' }}>
      ...
      <div style={{ display: 'flex', background: 'var(--surface-2)', borderBottom: '1px solid var(--border)', padding: '0 24px', flexShrink: 0, overflowX: 'auto' }}>
        {MARKET_TABS.map(tab => (
          <div key={tab} style={{ padding: '10px 16px', fontSize: 12, fontWeight: 600, color: tab === 'Deal Effectiveness' ? 'var(--accent)' : 'var(--text-3)', cursor: 'pointer', borderBottom: tab === 'Deal Effectiveness' ? '2px solid var(--accent)' : '2px solid transparent', whiteSpace: 'nowrap', flexShrink: 0 }}>
            {tab}
          </div>
        ))}
      </div>
